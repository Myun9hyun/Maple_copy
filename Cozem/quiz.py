...
# pdf_path = "Cozem/rule/아기자기_길드_규정_2023.pdf"
        ...
        if quiz10_password == password10:
            ...
            if st.button("정답 확인", key = "check_answer_button10"):
                if quiz10 == answer10:
                    ...
                    st.write("[뱐뎅]님의 기여도는 무려 340만이 넘어!┗|｀O′|┛")
                    st.write("기여도는 매일 5천까지만 채울 수 있으니 적어도 680일 넘게 기여도를 채워주셨어!")
    ...
    with tab3:
        ...
        if quiz11_password == password11:
            ...
            st.write("아래의 입력 창에 문제를 끝까지 풀어준 아깅이들의 닉네임을 적어줘!")
            st.write("실물 상품을 배송해주는 만큼, 닉네임을 적는 인원들은 주소도 꼭 입력해줘!")
            st.write("주소를 알려주지 않는 인원들은 아쉽지만 상품을 배송해 줄수 없어 추첨에서는 제외돼(T_T)")
